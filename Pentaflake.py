...
# Lógica de parámetros según el modo seleccionado (Rango extendido a 7)
if modo == "Visualización Estática":
    st.sidebar.header("Parámetros de Control")
    iteraciones = st.sidebar.slider("Iteraciones", min_value=0, max_value=7, value=3)
...
# --- FILTRO DE SEGURIDAD PARA ITERACIÓN CRÍTICA (NIVEL 7) ---
ejecutar_render = True
if iteraciones == 7 or (modo == "Animación de Construcción" and iter_final == 7):
    st.warning("Advertencia: La séptima iteración requiere un alto consumo de procesamiento y memoria volumétrica. La aplicación podría experimentar retardos significativos o inestabilidad en el navegador.")
    confirmacion = st.radio("¿Desea continuar con la ejecución de este orden fractal?", ["No", "Sí"], index=0)
    if confirmacion == "No":
        ejecutar_render = False
        st.info("Visualización geométrica suspendida. Seleccione un nivel de iteración entre 0 y 6 para un rendimiento óptimo o confirme la continuación.")
...
# --- CONTENEDOR INTEGRADO CON BOTONES DE COPIAR Y DESCARGAR NATIVOS (JS) ---
...
if ejecutar_render:
    if modo == "Visualización Estática":
        ...
        with col_btn1:
            boton_play = st.button("Iniciar Animación", use_container_width=True)
        ...
        st.markdown("---")
        st.subheader("Generación de Archivo de Animación (GIF)")
...
with col_graf1:
    ...
    st.caption("Gráfica exponencial ascendente: El perímetro diverge hacia el infinito debido a la autosemejanza recursiva.")

with col_graf2:
    st.markdown("**Comportamiento del Área ($A_n \\to 0$)**")
    st.line_chart(data=datos_area, color="#27AE60", use_container_width=True)
    st.caption("Gráfica de decaimiento: El área disminuye exponencialmente al remover los vacíos estructurales del polígono.")